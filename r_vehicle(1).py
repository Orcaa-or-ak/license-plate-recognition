...
# Frame path
frpath = org + '/result/frame'
if not os.path.exists(frpath): #Check if path exists
    os.mkdir(frpath)
...
for result in model.track(source=vid, conf=0.45, save=False, show=False, stream=False):
    cr_save = result.orig_img  
    fr_save = result.orig_img
    det = result.boxes
    fr += 1
    print("Frame:", fr)
    ...
    if len(det) > 0:
        try:
            for i in range(len(det)):
                ...
                #Crop
                c = c + 1
                cropframe = cr_save[ymin:ymax,xmin:xmax] #Start y : end y, start x : end x
                savestr = str(c) + ".jpg"
